...
#
# Installs the WordPress test suite for integration testing: downloads
# WordPress core and the WP core test library, and creates the test
# database. Location-independent — safe to invoke directly from
# vendor/silverassist/wp-coding-standards/scripts/install-wp-tests.sh in
# a consuming plugin, no path parameterization needed.
#
# Usage:
...
install_test_suite() {
	...
	if [ ! -d $WP_TESTS_DIR ]; then
		...
		# data/ directory was removed in WP 7.x; ignore if it doesn't exist.
		svn export --quiet --ignore-externals https://develop.svn.wordpress.org/${WP_TESTS_TAG}/tests/phpunit/data/ $WP_TESTS_DIR/data || true
	fi
